...
대전이 끝나면 클라이언트가 **이전 매칭 행을 정리**(`leave_queue`)한 뒤 **같은 스코프**(같은 반 / 전체)로 대기열에 다시 들어간다.

- 결과 화면에 **20초 카운트다운** 후 자동으로 새 상대를 찾는다. 「새 상대 찾기」로 즉시 재매칭할 수 있다.
...
- 오목·기울기 알까기·사각형 만들기·정사각형 만들기는 게임별 `pm_*_ratings` / `pm_*_apply_rating`을 쓴다 (단판 점수 규칙은 각 게임 구현 참고).
...
| 2026-09-10 | `g3-u3-2-starlight-seal` 원의 성질(현·접선) 솔로 게임 추가 |
| 2026-09-10 | `g3-u3-2-starlight-seal` 드래그로 거리·길이를 맞추는 「현·접선 거리 챌린지」로 전면 개편 |
| 2026-09-10 | `g3-u3-2-starlight-seal` 폐기 · 카탈로그·플레이 페이지·컴포넌트 제거 |
