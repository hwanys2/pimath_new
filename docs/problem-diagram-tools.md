...
| 3 | **기본** | 빠른 그림(프리셋), 도형·치수·단위·미지수. **그림 스타일**은 `<details>`로 접는다. |
...
5. **스타일은 3열에서 접어 둔다.** 선 굵기, 여백, 저장 배율. 글자 크기는 접힌 패널 안 슬라이더.
...
4. `components/tools/figures/<toolId>/`에 스튜디오 UI. **§5.1 3열**을 그대로 복사한다. 1열 그림, 2열 표시 칩·목록·보기, 3열 빠른 그림·치수·접힌 그림 스타일. 헤더에 PNG·복사. 수직선처럼 축이 가로로 긴 경우만 2열 예외.
...
- 각기둥·각뿔·각뿔대·원기둥·원뿔·원뿔대·정다면체. 숨은 모서리는 점선.
- 기본은 연한 무채색 면 채움. 체크를 끄면 흰 배경·검은 선만.
- 꼭짓점 이름, 높이·반지름·모선(옆면 모서리)·밑면 한 변, 모서리를 눌러 붙이는 설명선.
- 뿔·뿔대는 높이 또는 모선 중 어느 쪽 숫자를 고쳐도 그림이 맞춰진다.
- 빈 곳을 끌어 돌리고 「표준 보기」로 리셋. PNG·복사·SVG.
- 스튜디오는 §5.1 **기준 3열**: 그림 | 표시·보기 | 빠른 그림·도형·접힌 스타일.
