...
- 계급 시작·끝(`ㅁ ~ ㅁ`), 계급 크기(범위의 약수 버튼), 가로·세로축 이름, 세로 눈금. 시작이 0보다 크면 x축 끊기(물결). 그림 위 제목은 켜고 입력한다. 제목은 네모칸 밖 상단. 축 이름은 좌표평면과 같이 x는 우측하단 우측정렬, y는 좌측상단. 제목·축 이름은 끌어 옮기고 글자 크기를 따로 둔다.
