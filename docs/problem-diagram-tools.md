...
- 꼭짓점을 고르면 **대변에 수선**을 켤 수 있다. 둔각이면 밑변을 점선으로 연장한다. 직각 위치를 「없음」으로 두면 예각·둔각 삼각형도 된다.
- 이등변직각, 모눈·면 채움·빗변 분할은 칩으로 켠다.
- 프리셋: a,b,c 호, 9·12·x, 24·x·25, 이등변직각, 모눈 3×2·2×2·4×3, 넓이 증명, 30·40 수선, 둔각 수선, 6×8 대각선.
- 스튜디오는 §5.1 **3열**: 그림 아래 캔버스(모눈·좌표) | 세부(표시·수선·직각 위치) | 빠른 그림·그림 스타일(기본 펼침).
