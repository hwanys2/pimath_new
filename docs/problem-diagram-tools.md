...
- 직각삼각형은 점을 끌면 직각이 유지되고, 길이·각 숫자를 넣으면 그림이 맞춰진다. 각 표시는 항상 내각. 그림에서 변 길이를 고치면 **그 변만** 실제 길이에 맞춘다(각이 켜져 있으면 모양을 유지하고 닮음으로 맞춘다). **길이 글자와 설명선(점선 호)은 각각 드래그**한다. 점선 호는 꼭짓점에 닿고, 변에 수직으로만 높이를 바꾼다.
- 단위원: 제1사분면 사분원, θ에 따라 B(cos θ, sin θ)·D(1, tan θ). **B 또는 D를 끌어** θ를 바꾼다(각은 소수점 첫째 자리). 축·직각·y축 점선·각 x°·여각 y°·z°·cos·sin·tan은 칩으로 따로 켠다.
- 점마다 이름 → 점 → 안보임을 독립적으로 켠다. 점 이름은 점 옆(바깥)에 둔다.
- 삼각형 넓이: 수선은 A·B·C에서 여러 개 동시에 켤 수 있다. 면 채움·수선 강조·밑변 연장(둔각). 사각형: 일반/평행사변형, 변 설명선·내각·대각선 BD(분홍).
