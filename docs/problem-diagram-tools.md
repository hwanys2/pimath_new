...
- 피제수·제수를 넣으면 몫·내림·빼기의 과정이 그려진다. 순환마디는 최대 30자리. 더 길면 30자리까지 그리고 생략한다.
...
- 스튜디오는 §5.1 **3열**: 그림(남은 폭을 채움) | 표시·그래프·좌표평면(조금 좁게) | 빠른 그림·그림 스타일(기본 펼침). 글자·식 기본값은 크게, 슬라이더는 겨냥도와 비슷하게 더 키울 수 있다.
