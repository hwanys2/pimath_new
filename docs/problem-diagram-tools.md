...
1. **흰 배경, 검은 선.** 장식 색·그라데이션·이모지는 그림 안에 넣지 않는다.  
   예외: **좌표평면**은 교과서처럼 연한 청록 격자와 분홍·청록 그래프를 쓴다. **닮은 평면도형**의 모눈은 교과서 모눈종이처럼 연한 청록 격자다. **제곱근 수직선**은 모눈·정사각형 면(연한 파랑)·삼각형 면(연한 분홍)·호(분홍)를 쓴다. **히스토그램·도수분포다각형**은 막대 채움과 비교 선에 청록·분홍을 쓴다. **상자수염 그림**은 상자 채움과 이름 알약에 파스텔 색을 쓴다. **일차부등식** 해의 범위는 꺾은선 아래를 연한 분홍 등으로 칠할 수 있다. **순환소수 나눗셈**은 나머지·순환마디를 교과서처럼 분홍·파랑·노란 칸으로 표시한다.
