...
- 중2 피타고라스·중1 다각형의 설명선(점선 호)·직각 네모·라벨 모드를 재사용한다. 직각삼각형은 점을 끌면 직각이 유지되고, 길이·각 숫자를 넣으면 그림이 맞춰진다.
- 단위원: 제1사분면 사분원, θ에 따라 B(cos θ, sin θ)·D(1, tan θ). B를 호 위에서 끌거나 각도 칸으로 θ 변경. 축·직각·y축 점선·각 x°·여각 y°·z°는 칩으로 켠다.
- 삼각형 넓이: 수선 꼭짓점(A/B/C) 선택, 면 채움·수선 강조·밑변 연장(둔각). 사각형: 일반/평행사변형, 변 설명선·내각·대각선 BD(분홍).
