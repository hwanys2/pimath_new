...
1. **흰 배경, 검은 선.** 장식 색·그라데이션·이모지는 그림 안에 넣지 않는다.  
   예외: **좌표평면**은 교과서처럼 연한 청록 격자와 분홍·청록 그래프를 쓴다. **닮은 평면도형**의 모눈은 교과서 모눈종이처럼 연한 청록 격자다. **제곱근 수직선**은 모눈·정사각형 면(연한 파랑)·삼각형 면(연한 분홍)·호(분홍)를 쓴다. **히스토그램·도수분포다각형**은 막대 채움과 비교 선에 청록·분홍을 쓴다. **상자수염 그림**은 상자 채움과 이름 알약에 파스텔 색을 쓴다. **산점도**는 교과서처럼 분홍 점과 강조 검정 점을 쓴다. **일차부등식** 해의 범위는 꺾은선 아래를 연한 분홍 등으로 칠할 수 있다. **순환소수 나눗셈**은 나머지·순환마디를 교과서처럼 분홍·파랑·노란 칸으로 표시한다.
...
- **조작:** 원 둘레를 끌어 현을 그린다. 끝점 드래그는 길이 고정·원 위 회전. 끝점을 더블클릭하거나 중심까지 끌면 OA/OB를 잇는다. 선분(현 AB, 수선 OM, 반지름 OA/OB, 반길이 MB)을 직접 클릭하면 해당 변의 길이가 토글 표시된다. 길이 글자와 보조선(설명선 호)은 각각 따로 드래그한다. 설명선은 끌어서 높이만 바꾸고 끝점을 넘기지 않는다. 점 표시는 삼각비·원주각과 동일하게 4단 모드(점과이름 / 점만 / 이름만 / 안보임)와 개별 점(O, A, B, M) 순환 토글을 지원한다. 글자를 누르면 이름/숫자를 고친다. Delete·목록의 지우기로 현을 지운다.
- 수선·직각·OA/OB 등은 **2열** 칩. 원 반지름·단위는 같은 열. **3열**은 §5.1 원칙에 맞춰 **빠른 그림** + **그림 스타일**(기본 펼침). PNG/SVG 저장 시 `cropCanvasToInk`로 그림에 딱 맞게 여백(`EXPORT_INK_PAD = 12`)을 잘라낸다.

### 원주각 (`g3-inscribed-angles`)

중3 `3.2 원의 성질`에서 나오는 원주각·중심각 시험 그림.

- 그림 종류: **원주각·중심각**, **같은 호**, **지름**, **호와 원주각**, **내접사각형**, **접선과 현**.
- 원 위 점을 끌어 모양을 잡고, 두 번 누르면 반지름. 「점 찍기」로 점을 더한다. 고른 점에서 현·각·호 길이 칩.
...
중3 `3.1 삼각비`에서 나오는 전형적인 그림.

- 그림 종류: **직각삼각형**(삼각비 정의·특수각), **단위원**(sin·cos·tan), **삼각형의 넓이**(수선·예각·둔각), **사각형의 넓이**(일반·평행사변형·대각선).
- 직각삼각형은 점을 끌면 직각이 유지되고, 길이·각 숫자를 넣으면 그림이 맞춰진다. 각 표시는 항상 내각. 각을 고르면 **색(분홍·파랑·초록·회색·없음)** 과 **크기 숫자(숫자·x·숨김·직접)** 를 따로 켠다. 크기를 숨기면 호만 남는다. 각 크기 글자는 끌어 조금 옮긴다. **직각 네모는 그 각이 실제로 90°일 때만** 그린다. 점을 끌면 **보이는 길이·각 숫자가 실제 값으로** 바뀐다. 길이·각을 숫자로 넣으면 **그 값과 앞서 넣은 숫자는 유지**하고, 아직 고정하지 않은 변·각이 맞춰진다(미지수 `$x$` 등은 유지). 딱 떨어지면 빗변은 `$2\\sqrt{3}$`처럼 근호로 쓰고 소수 근사는 쓰지 않는다. **길이 글자와 설명선(점선 호)은 각각 드래그**한다. 점선 호는 꼭짓점에 닿고, 변에 수직으로만 높이를 바꾼다.
- 단위원: 제1사분면 사분원, θ에 따라 B(cos θ, sin θ)·D(1, tan θ). **B 또는 D를 끌어** θ를 바꾼다(각은 소수점 첫째 자리). 축·직각·y축 점선·각 x°·**여각 y°·z°(기본 숨김)**·cos·sin·tan은 칩으로 따로 켠다.
- 점마다 이름 → 점 → 안보임을 독립적으로 켠다. 점 이름은 점 옆(바깥)에 둔다.
- 삼각형 넓이: 수선은 A·B·C에서 여러 개 동시에 켤 수 있다. 면 채움·수선 강조·밑변 연장(둔각). **면 색**(분홍·파랑·초록·노랑)과 **수선 색**(분홍·파랑·초록)을 고른다. **빠른 그림의 밑변 AB는 가로**. 사각형: 일반/평행사변형, 변 설명선·내각·대각선 BD(분홍). 사각형도 면 색을 고른다.
- 프리셋: √3·3, 45° x·y, 30-60-90, 48° 단위원, 예각/둔각 높이, 60°·8 cm, 평행사변형, 대각선 BD 등.
- 스튜디오는 §5.1 **3열**: 그림 아래 종류 | 표시·보기(회전) | 빠른 그림·그림 스타일(기본 펼침).

### 상자수염 그림 (`g3-boxplot`)

중3 `4.2 상자그림과 산점도`에서 나오는 시험용 상자수염 그림(상자그림).

- 가로·세로. 상자 1~4개. 다섯 값(최솟값·Q1·중앙값·Q3·최댓값)을 그림에서 끌어 맞춘다. 칸에 숫자를 넣으면 이웃 값이 따라온다.
- 축 시작·끝·눈금·격자, 단위, 제목. 비교할 때는 이름 알약. 상자 위아래(가로)·좌우(세로) 여백을 맞춘다. 이름 색은 기본이 상자와 같고, 없음도 고를 수 있다.
- 프리셋: 가로 하나, 세로 나무 높이, 공장 비교, 반 나이.
- 스튜디오는 §5.1 **3열**: 그림 아래 방향·축 | 상자·표시 | 빠른 그림·그림 스타일(기본 펼침).

### 산점도 (`g3-scatter`)

중3 `4.2 상자그림과 산점도`에서 나오는 시험용 산점도.

- 그림 종류: **산점도** 하나, **상관 비교**(2×2). 양의·음의·없음·원형 구름을 개수·퍼짐만 넣고 다시 뿌린다.
- 축 시작·끝·눈금·격자, 가로·세로 이름, 원점 `$O$`, y축(·x축) 끊기(물결). 세로축 이름은 기본이 세로쓰기.
- 점: 구름 + 이름점(음식명 등) + 강조점(A, B, C…). 「점 찍기」로 그림에서 찍고, `이름 x y` 표를 붙여 넣을 수 있다. 점·글자는 끌어 옮긴다.
- 프리셋: 수심·용존산소(y축 끊기), 용량·가격(A~D), 상관 비교 2×2, 열량·지방(이름).
- 스튜디오는 §5.1 **3열**: 그림 아래 종류·축 | 구름·점·표시 | 빠른 그림·그림 스타일(기본 펼침).
...
- 변을 누르면 길이가 켜지고 다시 누르면 꺼진다. 칩으로 내각·외각(한 변 연장)·대각선을 켠다. 대각선은 **대각선 점선** 칩으로 실선/점선을 고른다. 외각 미지수는 교과서처럼 연한 분홍 채움을 켤 수 있다.
...
- 길이 표시 기본은 **숫자**(단위 `cm`). 미지수·직접 입력은 칩에서 고른다.
- 길이 글자와 설명선(점선 호)은 각각 드래그한다. 글자는 변 방향·수직으로 자유롭게 옮기고, 점선 호는 변에서 거리를 바꾼다.
- PNG·복사·SVG는 그림 잉크 기준으로 여백을 잘라 시험지에 붙이기 쉽게 한다.
...
- 중1 다각형의 점·각·길이·설명선과 중2 이등변의 등변 표시·각 채움을 재사용한다. **밑변 BC는 가로로** 두고, 꼭짓점을 끌면 고른 도형의 성질이 유지된다(평행사변형은 항상 평행사변형, 직사각형은 네 각 90°). **보기에서 대각선 교점 `$O$`를 중심으로 통째로 회전**할 수 있다.
- 길이 글자와 설명선(점선 호)은 각각 드래그한다. 변 연장은 꼭짓점·방향별로 여러 곳(DA→A, AB→B 등).
- 대변 길이, 대각·이웃각(분홍·파랑·초록·회색 채움), 같은 각 점·x 표시, 평행 화살, 두 대각선과 교점 `$O$`, 조각 길이 AO·OC·BO·OD, 삼각형 면 채움(△AOB·△BOC·△ODC·△OAD), 평행선 `$l$`·`$m$`.
...
- 스튜디오는 §5.1 **3열**: 그림+도형 | 보기(회전)·표시·면 채움 | 빠른 그림·그림 스타일(기본 펼침).
...
- 오른쪽은 회전(0–360°)·좌우/상하 대칭. 통째로 끌어 위치를 옮긴다. **보기에서 간격 슬라이더**로 두 도형을 더 붙이거나 띄운다. 모눈을 켜면 연한 청록 격자와 격자 맞춤.
- 변을 누르면 길이가 켜지고, 대응변·대응각 칩으로 짝에도 붙인다. 왼쪽 숫자를 넣으면 그 각·변만 맞추고 오른쪽은 닮음으로 따라온다.
- 프리셋: 삼각형 5:4, 삼각형 각·변, 사각형 8:10, 모눈 사각형 1:2 대칭.
- 스튜디오는 §5.1 **3열**(가로로 긴 쌍이라 1열이 남은 폭을 채움): 그림 아래 도형(변의 수·닮음비) | 표시·보기(회전·대칭·간격) | 빠른 그림·그림 스타일(기본 펼침).
...
- **길이 글자와 설명선(점선 호)은 각각 드래그한다.** 글자는 변을 따라·수직으로, 점선 호는 변에 수직인 방향으로만 높이를 바꾼다.
...
- 그림 종류: 직각삼각형(기호·숫자·x), 세 변 위 정사각형(모눈·ㄱㄴㄷ·분홍·파랑·노랑), 넓이 증명 `(a+b)²`, 빗변 수선, 사각형 대각선. **빠른 그림**이 곧 종류 선택이다.
- 중1 다각형의 설명선(점선 호)·직각 네모·라벨 모드를 재사용한다. 점을 끌면 직각이 유지되고, 길이 숫자를 넣으면 피타고라스로 맞춘다. **길이 글자와 설명선(점선 호)은 각각 드래그**한다. 점선 호는 변에 수직으로만 높이를 바꾼다.
- 꼭짓점을 고르면 **대변에 수선**을 켤 수 있다. 정사각형·빗변 수선·사각형에서도 같다. 둔각이면 밑변을 점선으로 연장한다. 직각 위치를 「없음」으로 두면 예각·둔각 삼각형도 된다.
- 1열 **캔버스**(모눈·점·이름)는 그림 종류와 무관하다. 빠른 그림을 바꿔도 모눈이 사라지지 않는다.
- 이등변직각, 면 채움·빗변 분할은 칩으로 켠다.
- 프리셋: a,b,c 호, 9·12·x, 24·x·25, 이등변직각, 모눈 3×2·2×2·4×3, 넓이 증명, 30·40 수선, 둔각 수선, 6×8 대각선.
- 스튜디오는 §5.1 **3열**: 그림 아래 캔버스(모눈·점) | 세부(표시·수선·직각 위치) | 빠른 그림·그림 스타일(기본 펼침).
...
| 2026-08-30 | 사각형: 대각선 교점을 중심으로 도형 전체 회전(보기 슬라이더). |
...
| 2026-08-30 | 평면도형의 닮음: 보기에서 두 도형 사이 간격(가깝게·보통·멀리)을 조절. |
...
| 2026-08-30 | 중3 삼각비 (`g3-trig-ratios`). 직각삼각형·단위원·넓이(삼각형·사각형), PNG. |
| 2026-08-30 | 삼각비 삼각형 넓이: 수선 A·B·C를 동시에 켤 수 있음. |
| 2026-08-30 | 삼각비: 그림에서 변 길이를 고치면 그 변의 실제 길이에 맞게 맞춘다. |
| 2026-08-30 | 삼각비: 직각 네모는 그 각이 실제로 90°일 때만. 4·6은 직각을 A에 표시. |
| 2026-08-30 | 삼각비: 계산된 빗변은 `$2\\sqrt{3}$`처럼 근호로 쓰고 소수 근사는 쓰지 않음. |
| 2026-08-30 | 삼각비: 각을 고르면 색과 크기 숫자를 따로 켠다. 크기를 숨기면 호만 남음. |
| 2026-08-30 | 삼각비: 면 채움·수선 강조 색을 분홍·파랑·초록(면은 노랑)에서 고른다. |
| 2026-08-30 | 삼각비: 점 이동 시 길이·각 숫자가 따라감. 직접 넣은 값은 나중에 다른 값을 넣어도 유지. |
| 2026-08-30 | 삼각비 삼각형 넓이 빠른 그림: 밑변 AB를 가로로. |
| 2026-08-30 | 삼각비 단위원: B·D로 θ 조작, 각은 소수점 첫째 자리, 각·cos·sin·tan을 따로 표시. |
| 2026-08-30 | 삼각비 단위원: 여각 y°·z°는 기본 숨김. |
| 2026-08-30 | 삼각비: 점 이름을 점 옆(바깥)에 둠. |
| 2026-08-30 | 삼각비 직각삼각형: 각 표시는 항상 내각. |
| 2026-08-30 | 삼각비: 각 크기 글자를 끌어 조금 옮길 수 있음. |
| 2026-08-30 | 중3 이차함수 그래프 (`g3-quadratic-function`). 포물선·꼭짓점·최댓값·평행이동·교점, PNG. |
| 2026-08-30 | 중3 상자수염 그림 (`g3-boxplot`). 가로·세로, 여러 상자, 다섯 값 드래그, PNG. |
| 2026-08-30 | 중3 산점도 (`g3-scatter`). 상관 구름·이름점·강조점·축 끊기·상관 비교 2×2, PNG. |
| 2026-08-30 | 상자수염: 값 화살표 제거. 상자 위아래·좌우 여백 슬라이더. |
| 2026-08-30 | 상자수염: 이름 색 기본은 상자와 같음. 없음 선택 가능. |
| 2026-08-30 | 중3 원주각 (`g3-inscribed-angles`). 원주각·중심각·같은 호·지름·호 길이·내접사각형·접선과 현, PNG. |
| 2026-09-01 | 이등변삼각형: PNG·SVG 여백을 그림에 맞게 자르고, 길이 표시 기본은 숫자(cm). |
| 2026-09-01 | 이등변삼각형: 길이 숫자와 점선 설명선을 따로 끌어 옮김. |
| 2026-09-03 | 다각형: 대각선을 점선으로 그리는 칩. |
| 2026-09-03 | 피타고라스: 설명선 호를 변에 수직으로만 드래그. 그림 종류를 빠른 그림에 합침. 꼭짓점 수선(둔각 연장선). |
| 2026-09-03 | 피타고라스: 1열 모눈·점·이름은 모든 그림에서 유지. 2열에서 점을 고르면 수선을 추가로 켬. |
| 2026-09-04 | 각 채움색: 삼각비·원주각·사각형에 회색을 더해 분홍·파랑·초록·회색 4종. |
