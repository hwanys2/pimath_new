...
- 각은 분홍·파랑 채움, 큰 각(180° 초과), 직각 네모. 호 길이는 원 밖 설명선. 접선·변 연장.
...
- 직각삼각형은 점을 끌면 직각이 유지되고, 길이·각 숫자를 넣으면 그림이 맞춰진다. 각 표시는 항상 내각. 각을 고르면 **색(분홍·파랑·초록·없음)** 과 **크기 숫자(숫자·x·숨김·직접)** 를 따로 켠다. 크기를 숨기면 호만 남는다. 각 크기 글자는 끌어 조금 옮긴다. **직각 네모는 그 각이 실제로 90°일 때만** 그린다. 점을 끌면 **보이는 길이·각 숫자가 실제 값으로** 바뀐다. 길이·각을 숫자로 넣으면 **그 값과 앞서 넣은 숫자는 유지**하고, 아직 고정하지 않은 변·각이 맞춰진다(미지수 `$x$` 등은 유지). 딱 떨어지면 빗변은 `$2\\sqrt{3}$`처럼 근호로 쓰고 소수 근사는 쓰지 않는다. **길이 글자와 설명선(점선 호)은 각각 드래그**한다. 점선 호는 꼭짓점에 닿고, 변에 수직으로만 높이를 바꾼다.
