...
- 단원·모험 게임 카드 **랭킹보기**: 전체 개인 최고 5위 모달. RPC `pm_list_public_game_ranking` (로그인 없이 조회, 타학교 이름 마스킹).
- 레거시 `pm_list_class_game_ranking`는 `scope='class'` 위임용으로만 유지한다.

### 3.2 명예의 전당 · 누적 XP 랭킹

공개 메인(`/`)·학생 `/adventure`·교사 `/teacher`에서 **누적 `total_xp`** 로 전체·학교·학급 순위를 보여 준다. 교사 학급 상세의 **포인트 순위** 탭은 그 학급 학생만 시상대+2열로 보여 준다.
...
| **학교별** | 학교 XP 합 대항전(학교별 랭킹). 학교를 누르면 그 학교 학생 개인 순위(내 등수 근처) |
...
- RPC: `pm_list_hof_schools` / `pm_list_hof_classes` / `pm_list_hof_students` / `pm_get_hof_viewer`. 학교별 랭킹(학교 합산)과 학교 클릭 시 그 학교 학생 순위(상위 8 + 내 등수 ±3, 내 이름 강조).
...
| 2026-08-28 | `/teacher`는 우리학교·새학급 | 명예의 전당. 학급 탭 순위는 해당 반만 시상대 |
| 2026-09-02 | 게임 랭킹 타학교 이름 마스킹 · 교사 대시보드 학급/학교/전체 동시 표시 |
| 2026-09-02 | 단원 게임 카드 랭킹보기 — 전체 5위 미리보기 |
| 2026-09-03 | 학교별 랭킹 라벨 · 학교 클릭 시 상위 8 + 내 등수 ±3 · 어드벤처 캐릭터 칸 넓은 화면 레이아웃 |
