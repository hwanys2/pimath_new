-- pm_sq: square-maker matchmaking + games + ratings (pimath only)

CREATE TABLE IF NOT EXISTS public.pm_sq_queue (
  ...
  CONSTRAINT pm_sq_queue_player_key_nonempty CHECK (length(trim(player_key)) > 0)
);

CREATE UNIQUE INDEX IF NOT EXISTS pm_sq_queue_one_waiting
  ON public.pm_sq_queue (player_key)
  WHERE status = 'waiting';

CREATE INDEX IF NOT EXISTS pm_sq_queue_match_idx
  ON public.pm_sq_queue (scope, class_id, status, created_at)
  WHERE status = 'waiting';

CREATE TABLE IF NOT EXISTS public.pm_sq_games (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  game_phase text NOT NULL DEFAULT 'rps'
    CHECK (game_phase IN ('rps', 'playing')),
...
CREATE INDEX IF NOT EXISTS pm_sq_games_active_idx
  ON public.pm_sq_games (status, updated_at)
  WHERE status = 'playing';

CREATE TABLE IF NOT EXISTS public.pm_sq_ratings (
...
CREATE INDEX IF NOT EXISTS pm_sq_ratings_score_idx
  ON public.pm_sq_ratings (total_score DESC);

DO $$ BEGIN
  ALTER TABLE public.pm_sq_queue
    ADD CONSTRAINT pm_sq_queue_game_fk
    FOREIGN KEY (game_id) REFERENCES public.pm_sq_games(id) ON DELETE SET NULL;
EXCEPTION
  WHEN duplicate_object THEN NULL;
END $$;

ALTER TABLE public.pm_sq_queue ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.pm_sq_games ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.pm_sq_ratings ENABLE ROW LEVEL SECURITY;

-- ---------------------------------------------------------------------------
-- Identity
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.pm_sq_resolve_identity(
...
-- RPS: a beats b?
CREATE OR REPLACE FUNCTION public.pm_sq_rps_beats(p_a text, p_b text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
...
CREATE OR REPLACE FUNCTION public.pm_sq_expand_queue_global(
  p_session_token text,
  p_guest_id text
)
RETURNS TABLE (
...
BEGIN
  ...
  IF v_qid IS NULL THEN
    ...
    FROM public.pm_sq_join_queue(p_session_token, p_guest_id, 'global') j;
    RETURN;
  END IF;
...
-- RPS + shape pick
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.pm_sq_submit_rps(
  p_session_token text,
  p_guest_id text,
  p_game_id uuid,
  p_choice text
...
BEGIN
  ...
  IF v_g.black_key = v_key THEN
    UPDATE public.pm_sq_games SET rps_black = p_choice, updated_at = now() WHERE id = p_game_id;
    v_g.rps_black := p_choice;
  ELSE
    UPDATE public.pm_sq_games SET rps_white = p_choice, updated_at = now() WHERE id = p_game_id;
  ...
  IF public.pm_sq_rps_beats(v_g.rps_black, v_g.rps_white) THEN
  ...
  ok := true; game_phase := 'playing'; rps_tie := false; error_code := NULL;
  RETURN NEXT;
END;
$$;
...
-- Ratings
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.pm_sq_delta_for_outcome(
  p_total int,
  p_outcome text
)
RETURNS int
...
BEGIN
  ...
  v_delta := public.pm_sq_delta_for_outcome(v_before, p_outcome);
...
CREATE OR REPLACE FUNCTION public.pm_sq_list_rating_ranking(
  p_session_token text,
  p_scope text DEFAULT 'class'
)
RETURNS TABLE (
...
GRANT EXECUTE ON FUNCTION public.pm_sq_join_queue(text, text, text) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.pm_sq_expand_queue_global(text, text) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.pm_sq_leave_queue(text, text) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.pm_sq_poll(text, text, uuid) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.pm_sq_submit_rps(text, text, uuid, text) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.pm_sq_apply_move(text, text, uuid, int, int, jsonb, text, text, int) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.pm_sq_timeout_apply_move(text, text, uuid, int, int, jsonb, text, text, int) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.pm_sq_claim_result(text, text, uuid) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.pm_sq_apply_rating(text, text) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.pm_sq_list_rating_ranking(text, text) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.pm_sq_delta_for_outcome(int, text) TO anon, authenticated;
