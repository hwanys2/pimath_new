...
function softMessage(notice: SoftNotice, table: boolean): string {
  switch (notice.reason) {
    ...
    case "wrong":
      return table
        ? "색이 표시된 칸을 다시 재어 보세요."
        : "다시 재어 보세요. 그린 삼각형의 비를 실제 빗변에 곱하면 돼요.";
  }
}
  ...
  return (
    <section className="quest-card-static p-4 sm:p-6">
      ...
      <p className="mt-3 text-sm font-semibold text-foreground/75">
        {table
          ? "같은 일을 여러 번 하니 번거롭지요. 각마다 높이÷빗변과 밑변÷빗변을 표로 모아 봅시다. 오른쪽에서 빗변을 먼저 그리면 점선 원이 생깁니다. 빗변을 10칸으로 두면 높이와 밑변이 곧 (비)×10이라 계산이 편해요."
          : scene?.prompt}
      </p>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <div className="min-w-0">
              ...
              <table className="w-full min-w-[18rem] text-sm">
                <thead>
                  <tr className="border-b border-wood/15 bg-lavender/30 text-wood">
                    ...
                    <th className="px-3 py-2 text-left font-bold">높이 ÷ 빗변</th>
                    <th className="px-3 py-2 text-left font-bold">밑변 ÷ 빗변</th>
                  </tr>
                </thead>
                <tbody>
                      ...
                      <tr key={angle} className="border-b border-wood/8">
                        ...
                        <td className="px-2 py-1.5">
                          <input
                            ...
                            spellCheck={false}
                            disabled={locked}
                            aria-label={`${angle}도에서 높이 나누기 빗변`}
                        ...
                        <td className="px-2 py-1.5">
                          <input
                            ...
                            spellCheck={false}
                            disabled={locked}
                            aria-label={`${angle}도에서 밑변 나누기 빗변`}
          ...
          <label className="mt-4 block">
            ...
            <textarea
              ...
              placeholder={
                table
                  ? "예: 각도마다 빗변 10칸 직각삼각형을 그려 높이÷빗변, 밑변÷빗변을 구했어요."
        ...
        <div className="min-h-[22rem] min-w-0">
          <SincosSketchpad key={stepIndex} locked={locked && !hostPreview} />
        </div>
      </div>
        ...
        >
          ...
          {table && submitFeedback === "correct" ? (
            <p className="mt-2 text-sm font-bold leading-relaxed">
              높이÷빗변이 <span className="text-[#6b4a9e]">sin(각)</span>, 밑변÷빗변이{" "}
              <span className="text-[#6b4a9e]">cos(각)</span>입니다. 각마다 비를 모아
              두면, 매번 삼각형을 그리지 않아도 높이와 거리를 구할 수 있어요.
            </p>
          ) : (
