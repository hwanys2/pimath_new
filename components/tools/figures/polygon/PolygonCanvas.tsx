...
import { type PolygonState } from "@/lib/diagrams/polygon/model";
...
import {
  ...
  SCENE_WIDTH,
  type PolygonScene,
} from "@/lib/diagrams/polygon/scene";
...
export type PolygonSetter = (
  updater: PolygonState | ((prev: PolygonState) => PolygonState),
  persist?: boolean,
) => void;
...
type Props = {
  state: PolygonState;
  fonts: FontFaces;
  selected: PolygonSelection | null;
  setState: PolygonSetter;
  persist: () => void;
  onSelect: (sel: PolygonSelection | null) => void;
...
export default function PolygonCanvas({
  ...
  const sceneRef = useRef<PolygonScene | null>(null);
  const dragRef = useRef<Drag | null>(null);
  const hoverRef = useRef<PolygonHit | null>(null);
  ...
  const paint = useCallback(() => {
    ...
    const scene = buildPolygonScene(current);
...
function cursorForHit(hit: PolygonHit | null): string {
...
function paintOverlays(
  ...
  scene: PolygonScene,
  hover: PolygonHit | null,
  selected: PolygonSelection | null,
) {
  ctx.save();
  const verts = scene.layout.canvas;
  const n = verts.length;
  function ring(i: number, strong: boolean) {
    const p = verts[i];
    if (!p) return;
    ctx.beginPath();
  ...
  if (selected?.t === "vertex") ring(selected.i, true);
  if (selected?.t === "edge") edge(selected.i);
  if (hover?.kind === "vertex" && selected?.t !== "vertex") ring(hover.index, false);
  if (hover?.kind === "vertex" && selected?.t === "vertex" && selected.i !== hover.index) {
    ring(hover.index, false);
  }
  if (hover?.kind === "edge" && !(selected?.t === "edge" && selected.i === hover.index)) {
    edge(hover.index);
  }
  ctx.restore();
}
