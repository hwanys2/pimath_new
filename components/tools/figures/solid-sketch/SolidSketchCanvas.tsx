...
import { type SolidSketchState } from "@/lib/diagrams/solid-sketch/model";
...
export type SolidSketchSetter = (
  updater: SolidSketchState | ((prev: SolidSketchState) => SolidSketchState),
  persist?: boolean,
) => void;
...
type Props = {
  ...
  fonts: FontFaces;
  setState: SolidSketchSetter;
  persist: () => void;
};

const MOVE_PX = 3;

export default function SolidSketchCanvas({
  state,
  fonts,
  setState,
  persist,
  ...
  const hoverRef = useRef<SolidHit | null>(null);
  ...
  const paint = useCallback(() => {
    ...
    const scene = buildSolidSketchScene(current);
  ...
  function hitAt(e: { clientX: number; clientY: number }) {
    ...
    return hitTestSolid(stateRef.current, scene, p.x, p.y, hitScale());
  }
    ...
    <div className="relative">
      <canvas
        ...
        onPointerDown={(e) => {
          ...
          if (hit?.kind === "edge") {
            setState((prev) => toggleEdgeMeasure(prev, hit.key), true);
            return;
          }
        ...
        onPointerMove={(e) => {
          ...
          if (drag.t === "orbit") {
            ...
            if (moved) {
              setState((prev) => orbitView(prev, dx, dy), false);
            }
            return;
          }
          dragRef.current = { ...drag, x: p.x, y: p.y, moved };
          ...
          if (!scene) return;
          setState(
            (prev) => nudgeMeasure(prev, scene, drag.id, dx, dy, drag.t === "dimLine"),
            false,
          );
        }}
        onPointerUp={(e) => {
...
function sameHit(a: SolidHit | null, b: SolidHit | null): boolean {
  if (a === b) return true;
  ...
  if (a.kind === "edge") return b.kind === "edge" && a.key === b.key;
  return true;
}

function cursorForHit(hit: SolidHit | null): string {
  if (!hit) return "grab";
...
function paintOverlays(
  ...
  if (hover.kind === "vertex") {
    const p = scene.layout.vertices[hover.index];
    if (p) {
      ctx.beginPath();
  ...
  if (hover.kind === "edge") {
    const e = scene.layout.edges.find((item) => item.key === hover.key);
    if (e) {
