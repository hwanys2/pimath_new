...
import {
  ...
  projectOnCircle,
  rotateSector,
  rotateSectorEndpoint,
  sectorMath,
} from "@/lib/diagrams/circle-sectors/geometry";
import type { CircleSectorsState } from "@/lib/diagrams/circle-sectors/model";
import {
  buildCircleSectorsScene,
  canvasToMath,
  hitTestFigure,
  mathToCanvas,
  measureFrame,
  ...
  type DiagramScene,
  type FigureHit,
} from "@/lib/diagrams/circle-sectors/scene";
...
type Drag =
  ...
  | { t: "endpoint"; sectorId: string; which: "start" | "end" }
  | { t: "spin"; sectorId: string; lastDeg: number }
  | { t: "view"; lastX: number; lastY: number }
  | { t: "draw"; a: { x: number; y: number }; b: { x: number; y: number } };

export type CircleSectorsSetter = (
  updater:
    | CircleSectorsState
    | ((prev: CircleSectorsState) => CircleSectorsState),
  persist?: boolean,
) => void;
...
export default function CircleSectorsCanvas({
  state,
  fonts,
  tool,
  selectedId,
  ...
  const paint = useCallback(() => {
    ...
    const scene = buildCircleSectorsScene(current);
  ...
  function finishSector(
    ...
    const added = addSectorFromDrag(stateRef.current, a, b);
    if (!added) return false;
    setState(() => added.state, true);
    onSelect(added.id);
    onToolChange("select");
    pendingDrawRef.current = null;
    return true;
  ...
  function startDraw(math: { x: number; y: number }) {
    if (stateRef.current.sectors.length >= 4) return;
    const point = projectOnCircle(math, stateRef.current.radius);
    if (pendingDrawRef.current) {
      finishSector(pendingDrawRef.current, point);
      dragRef.current = null;
      paint();
      return;
    }
  ...
  return (
    <div className="relative">
      <canvas
        ...
        tabIndex={0}
        aria-label="원과 부채꼴. 원 위를 끌어 부채꼴을 그리고, 점을 옮기거나 글자를 눌러 바꿀 수 있어요."
        onPointerDown={(e) => {
          ...
          if (hit.kind === "label") {
            ...
            setCursor("grabbing");
            e.currentTarget.setPointerCapture(e.pointerId);
            const sectorId = sectorIdFromLabel(hit.id);
          ...
          if (hit.kind === "sector") {
            ...
            dragRef.current = {
              t: "spin",
              sectorId: hit.sectorId,
              lastDeg: (Math.atan2(math.y, math.x) * 180) / Math.PI,
            };
            setCursor("grabbing");
            e.currentTarget.setPointerCapture(e.pointerId);
            return;
        ...
        onPointerMove={(e) => {
          ...
          if (drag.t === "label" || drag.t === "dimLine") {
            ...
            if (moved) {
              ...
              if (frame && parsed) {
                ...
                setState(
                  (prev) =>
                    mapSector(prev, parsed.sectorId, (s) => ({
                      ...s,
                      [parsed.key]: nudge(
                        s[parsed.key as "angleLabel"],
                        dx,
                        dy,
                        frame.along,
                        frame.outward,
          ...
          if (drag.t === "endpoint") {
            setState(
              (prev) =>
                mapSector(prev, drag.sectorId, (sector) =>
                  rotateSectorEndpoint(sector, math, drag.which),
                ),
              false,
            );
            return;
          ...
          if (drag.t === "spin") {
            const deg = (Math.atan2(math.y, math.x) * 180) / Math.PI;
            const delta = deg - drag.lastDeg;
            dragRef.current = { t: "spin", sectorId: drag.sectorId, lastDeg: deg };
            setState(
              (prev) =>
                mapSector(prev, drag.sectorId, (sector) =>
                  rotateSector(sector, delta),
                ),
              false,
            );
            return;
...
function sameHit(a: FigureHit | null, b: FigureHit | null): boolean {
  ...
  if (a.kind === "point") {
    return (
      b.kind === "point" && a.sectorId === b.sectorId && a.which === b.which
    );
  }
  if (a.kind === "sector") {
    return b.kind === "sector" && a.sectorId === b.sectorId;
  }
  return true;
}

function sectorIdFromLabel(id: string): string | null {
  return parseMeasureId(id)?.sectorId ?? null;
}
...
function paintOverlays(
  ...
  for (const sector of state.sectors) {
    ...
    if (selected) {
      ...
      ctx.lineWidth = 2.2;
      ctx.beginPath();
      ctx.moveTo(scene.layout.origin.x, scene.layout.origin.y);
    ...
    for (const handle of [
      { p: cA, which: "start" as const },
      { p: cB, which: "end" as const },
    ]) {
      const hovered =
        hover?.kind === "point" &&
        hover.sectorId === sector.id &&
