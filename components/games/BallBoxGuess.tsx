...
/** Build a jsonb-safe answer/guess object from string inputs. */
...
export default function BallBoxGuess({
  ...
  // Teacher composition inputs (per color: "" = exclude, "0" = decoy, ">0" = in box)
  ...
  const poll = useCallback(async () => {
    ...
    const result = isTeacher
      ? await ballBoxTeacherPollAction({ sessionId })
      : await ballBoxStudentPollAction({ sessionId });
  ...
  // Reset per-round student UI when a new round starts.
  useEffect(() => {
  ...
  // Teacher: reconnect to an active session.
  useEffect(() => {
  ...
  // Student: find + join an active session.
  useEffect(() => {
  ...
  const handleDraw = async () => {
    ...
    const res = await ballBoxDrawAction({ sessionId });
    ...
    return (
      <div className="rounded-2xl border border-wood/10 bg-peach/20 p-8 text-center">
        ...
        <p className="mt-2 text-sm text-foreground/60">
          교사는 상자를 설정하고, 학생은 같은 반에서 접속해 공을 뽑으며 개수를
          맞혀요.
        </p>
      </div>
    );
      ...
      {sessionId && (
        <div className="grid gap-5 lg:grid-cols-[1fr_280px]">
          <div className="space-y-5">
            ...
            {/* Student: waiting inside a lobby session */}
            {isStudent && state.phase === "lobby" && (
              <div className="rounded-2xl border border-wood/10 bg-peach/20 p-8 text-center">
                <p className="font-display text-xl text-wood">
                  교사가 상자를 설정하고 있어요
                </p>
                ...
                {!state.mySolved ? (
                  <div className="space-y-3 rounded-2xl border border-wood/10 bg-white/70 p-5">
                    ...
                    {feedback && !feedback.correct && (
                      <p className="rounded-xl bg-red-50 px-4 py-2 text-sm text-red-700">
                        아쉬워요, 아직 정답이 아니에요. 더 뽑아 보고 다시
                        도전하세요! (재도전할수록 점수는 조금 낮아져요)
                      </p>
                    )}
                    <button
                      type="button"
                  ...
                  <div className="rounded-2xl border border-gold/40 bg-gold/20 p-5 text-center">
                    ...
                    <p className="mt-2 text-sm text-foreground/70">
                      {state.myDrawCount}번 만에 맞혔어요! 정답 공개를 기다려
                      주세요.
                    </p>
                  </div>
                )}
