...
export async function applySqRatingFromSession(input: {
  ...
  if (error) {
    console.error("[pm] pm_sq_apply_rating:", error.message);
...
export async function fetchSqRatingRanking(input: {
  scope?: RankingScope;
  sessionToken?: string | null;
}): Promise<RankingRow[]> {
  const token = input.sessionToken ?? (await getStudentSessionToken());
  ...
  const supabase = await createClient();
  const { data, error } = await supabase.rpc("pm_sq_list_rating_ranking", {
    p_session_token: token,
    p_scope: scope,
  });

  if (error) {
    console.error("[pm] pm_sq_list_rating_ranking:", error.message);
    return [];
  }
