...
export async function applyQuadRatingFromSession(input: {
  outcome: QuadOutcome;
  sessionToken?: string | null;
}): Promise<QuadRatingResult | { error: string }> {
  ...
  const { data, error } = await supabase.rpc("pm_quad_apply_rating", {
    p_session_token: token,
    p_outcome: input.outcome,
  });

  if (error) {
    console.error("[pm] pm_quad_apply_rating:", error.message);
...
export async function fetchQuadRatingRanking(input: {
  scope?: RankingScope;
  sessionToken?: string | null;
}): Promise<RankingRow[]> {
  const token = input.sessionToken ?? (await getStudentSessionToken());
  ...
  const supabase = await createClient();
  const { data, error } = await supabase.rpc("pm_quad_list_rating_ranking", {
    p_session_token: token,
    p_scope: scope,
  });

  if (error) {
    console.error("[pm] pm_quad_list_rating_ranking:", error.message);
    return [];
  }
