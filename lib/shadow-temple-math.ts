...
function buildLavaRoom(): Room {
  ...
  return {
    ...
    enterStory: [
      "문을 지나는 순간, 발밑의 바닥이 콰르릉 무너져 내린다!",
      "가까스로 좁은 기둥 P 위로 뛰어올랐다. 저 아래에서 용암이 부글부글 끓어오른다.",
    ...
    puzzles: [
      {
        ...
        hintSolve: steps,
        solvedLine:
          "로프가 번쩍이며 정확히 발판 B 에 꽂힌다. 용암 위를 미끄러져 무사히 건넜다!",
        weight: 1,
      },
    ],
...
function buildBridgeRoom(): Room {
  ...
  return {
    ...
    enterStory: [
      "이번엔 칠흑 같은 낭떠러지. 다리는 오래전에 끊어져 잔해만 매달려 있다.",
...
function buildShieldRoom(): Room {
  ...
  return {
    ...
    enterStory: [
      "다리를 건너자 거대한 수호자 동상이 길을 막는다. 두 눈이 침입자를 노려본다.",
      "동상의 가슴에 평행사변형 모양의 홈이 파여 있고, 바닥에는 방패 네 개가 흩어져 있다.",
      "「넓이가 홈과 같은 방패만이 수호자의 심장을 채우리라.」 — 틀린 방패를 쥐여 주면… 무슨 일이 벌어질지 모른다.",
    ],
    puzzles: [
      {
...
export const PROLOGUE: string[] = [
  "새벽 4시. 실종된 수학자 피타 박사의 GPS 가 마지막으로 가리킨 곳 — 그림자 신전.",
  "박사가 남긴 낡은 탐사 수첩의 첫 장에는 이렇게 적혀 있다.",
  "「이 신전은 여섯 개의 시련으로 잠겨 있다. 열쇠는 검이 아니라 삼각비다.」",
  "당신은 수첩을 꼭 쥐고 신전에 발을 들인다. 그 순간 —",
  "쿠구구궁…!! 등 뒤에서 석문이 닫혔다. 횃불이 다 타기 전에, 여섯 개의 방을 모두 풀고 탈출하라!",
];

export const ESCAPE_STORY: string[] = [
  "황금의 별을 손에 쥐자 신전 전체가 요동친다. 무너지는 복도를 전력으로 내달린다!",
  "돌문이 열리고 — 쏟아지는 새벽빛. 그리고 그 빛 속에, 익숙한 실루엣이 서 있다.",
  "「하하, 드디어 풀었구나!」 실종된 줄 알았던 피타 박사다.",
  "「이 신전은 내가 만든 최종 시험이었단다. 삼각비를 검처럼 다루는 자, 이제 너는 나의 후계자다.」",
];

export const TRAPPED_STORY: string[] = [
  "마지막 횃불이 파르르 떨리다 — 꺼졌다. 신전이 깊은 어둠에 잠긴다.",
  "…그때, 어디선가 낮은 웃음소리와 함께 입구의 돌문이 스르륵 열린다.",
  "「오늘은 여기까지. 신전은 도망가지 않는단다.」 — 수첩에 새 글씨가 번져 나온다.",
  "다음엔 더 빠르게, 더 정확하게. 신전은 다시 도전하는 자를 기다린다.",
];
